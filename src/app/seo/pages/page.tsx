  ...
  useEffect(() => {
    async function fetchPages() {
      try {
        ...
        // adjust page+limit or wire up real pagination
        const res = await fetch(`${base}/page-seo?language=en&page=1&limit=100`, {
          headers: { Authorization: `Bearer ${Cookies.get('accessToken')}` },
        });
        if (!res.ok) throw new Error('Failed to fetch pages');
        const json = await res.json();
        // json: { items: PageSeo[], total, ... }
                ...
                {filteredPages.length ? (
                  filteredPages.map(page => (
                    <tr
                      ...
                      onClick={() => router.push(`/seo/pages/${page.id}/edit`)}
                    >
                      ...
                      <td className="flex justify-end px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        ...
                        />
                        <Trash size={16} className="text-red-600 hover:text-red-900" />
                      </td>
                    </tr>
                  ))
                ) : (
